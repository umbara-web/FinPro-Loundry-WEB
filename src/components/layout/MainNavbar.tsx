...
export function Navbar() {
  ...
  const menuItems =
    isLoggedIn && user?.role === 'CUSTOMER' ? customerLinks : navLinks;
              ...
              <nav className='z-10 hidden flex-1 items-center justify-center gap-2 lg:flex'>
                {menuItems.map((item) => (
                  <Link
                    key={item.label}
                    href={item.href}
                    className='text-primary px-4 py-2 text-sm font-medium transition-all hover:scale-105 hover:rounded-full hover:bg-gray-200 hover:shadow-lg dark:text-white dark:hover:bg-gray-600'
                  >
                    {item.label}
                  </Link>
                ))}
