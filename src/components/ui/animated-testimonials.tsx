...
export const AnimatedTestimonials = ({
  ...
  /* 
    Use a deterministic "random" value to avoid hydration mismatch errors.
    We just use the index to select a rotation. 
  */
        ...
        <div className="flex flex-col justify-between py-4">
          ...
          >
            ...
            <div className="flex gap-1 text-yellow-400 mt-2">
                {[...Array(Math.floor(testimonials[active].rating))].map((_, i) => (
                    <Star key={i} className="w-5 h-5 fill-current" />
                ))}
                {testimonials[active].rating % 1 !== 0 && <StarHalf className="w-5 h-5 fill-current" />}
            </div>
          ...
          <div className="flex gap-4 pt-12 md:pt-0">
            <button
              onClick={handlePrev}
              className="group/button flex h-7 w-7 items-center justify-center rounded-full bg-gray-200 dark:bg-neutral-700"
            >
