        ...
        <div className="w-full">
            <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-gray-800">
                  ...
                  ${order.status === 'WAITING_DRIVER' ? 'bg-yellow-500/10 text-yellow-500 border-yellow-500/20' :
                                        order.status === 'DRIVER_ASSIGNED' ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' :
                                            order.status === 'PICKED_UP' ? 'bg-purple-500/10 text-purple-400 border-purple-500/20' :
                                                order.status === 'ARRIVED_OUTLET' ? 'bg-green-500/10 text-green-400 border-green-500/20' :
                                                    order.status === 'CANCELLED' ? 'bg-red-500/10 text-red-400 border-red-500/20' :
                                                        'bg-gray-500/10 text-gray-400 border-gray-500/20'}`}>
                                    ● {order.status === 'WAITING_DRIVER' ? 'Waiting Driver' :
                                        order.status === 'DRIVER_ASSIGNED' ? 'Driver Assigned' :
                                            order.status === 'PICKED_UP' ? 'Picked Up' :
                                                order.status === 'ARRIVED_OUTLET' ? 'Arrived Outlet' :
